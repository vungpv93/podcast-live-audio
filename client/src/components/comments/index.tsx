...
const Comments: React.FC<ILiveAudio> = ({ roomId, socket, auth }) => {
  const { isReady, comments } = useComment({ roomId, socket });
  const [openActionId, setOpenActionId] = useState(null);
  const toggleActionMenu = (commentId) => {
    setOpenActionId((prev) => (prev === commentId ? null : commentId));
  };
